...
summary_table[6,]=c("model_1,both,sdb",
                    ...
                    "num_significant/768")
write.csv(summary_table,file="output/supplementary_tables1.csv",row.names=F)
...
write.csv(summary_superpathway_tbl,file="output/supplementary_tables2.csv",row.names=F)
 ...
 write.csv(summary_subpathway_category_tbl,file="output/supplementary_tables3.csv",row.names=F)
...
# Supplementary table s4
 # make a summary table for super_pathway and sleep trait category
  ...
  write.csv(summary_superpathway_category_tbl,file="output/supplementary_tables4.csv",row.names=F) 
 ...
 ggsave(circular_stacked_bar_plot, file = "output/figure4.png", width = 12, height = 10, units = "in") # Adjust dimensions as needed
 ...
 ggsave(ggarrange(circular_stacked_bar_plot_male,circular_stacked_bar_plot_female,common.legend=T), file = "output/supplementary_figures2.png", width = 24, height = 10, units = "in") # Adjust dimensions as needed
...
# Heatmap for metabolite super pathway by sleep trait categories
  heatmap_category<-ggplot(summary_superpathway_category_tbl, aes(x = super_pathway, y = category, fill = percent_significant)) +
   ...
   coord_flip()
  png("output/figure5.png", width = 500, height = 500)
...
top_metab_count<-metab_significant_associations_counts%>%
 ...
 metab_top_count_plot<-ggplot(metab_significant_associations_top_count, aes(x = chemical_name, y = count, fill = category)) +
   geom_bar(stat = "identity") +
   scale_fill_brewer(palette = "Set3") +  # Using "Set3" palette for fill colors
   theme_minimal() +
   labs(title = "Top 10% Metabolites by Significant Associations",
        x = "Metabolites",
        y = "Count of Significant Associations",
        fill = "Sleep Trait Category") +
   theme(axis.text.x = element_text(angle = 0, hjust = 1),
         plot.background = element_rect(fill = "white", colour = "white")) + coord_flip()
 ggsave(metab_top_count_plot, file = "output/figure6a.png", width = 12, height = 10, units = "in") # Adjust dimensions as needed
 # export the results in table
 write.csv(metab_significant_associations_top_count,file= "output/table3.csv",row.names = F)
 
 metab_top_count_superpathway_plot<-ggplot(metab_significant_associations_top_count, aes(x = chemical_name, y = count, fill = super_pathway)) +
   geom_bar(stat = "identity") +
   # scale_fill_brewer(palette = "Set3") +  # Using "Set3" palette for fill colors
   theme_minimal() +
   labs(title = "Top 10% Metabolites by Significant Associations",
   ...
   theme(axis.text.x = element_text(angle = 0, hjust = 1),
         plot.background = element_rect(fill = "white", colour = "white")) + coord_flip()
 ggsave(metab_top_count_superpathway_plot, file = "output/figure6b.png", width = 12, height = 10, units = "in") # Adjust dimensions as needed
...
ggsave(dice_matrix_category_metab, file = "output/figure7.png", width = 6, height = 5, units = "in") # Adjust dimensions as needed
...
geom_segment(data = link_counts,
  ...
  theme(plot.background = element_rect(fill = "white"),
        panel.grid = element_blank())
ggsave(Fruchterman_Reingold_network, file = "output/figure8.png", width = 14, height = 8, units = "in") # Adjust dimensions as needed
...
# amino acid
plotweb(both_md1_web$`Amino Acid`)
plotweb(both_md1_web$`Lipid`)
plotweb(both_md1_uni$one,
        col.low="grey50",col.interaction="wheat2") # combine all super pathway together
# conver to one-mode network (metabolites connectedness via sleep traits)
par(xpd=TRUE, mar=c(0, 6, 0, 6))
gplot(as.one.mode(both_md1_web$`Xenobiotics`, project="lower"),
      label=rownames(both_md1_web$`Xenobiotics`), gmode="graph",
      label.cex=0.6, vertex.cex=0.8,edge.col=3, label.pos=0,vertex.col="green")
...
write.csv(network_output,file="output/supplementary_tables5.csv")
...
network_properties_plot1<-ggplot(network_long1,aes(x=factor(super_pathway,levels=pathway_order),y=value,color=measure, group = measure))+
  ...
  theme(plot.title = element_text(hjust = 1),axis.text.x = element_text(size = 11))+
...
ggsave(network_properties_plot, file = "output/figure9.png", width = 12, height = 6, units = "in") # Adjust dimensions as needed
