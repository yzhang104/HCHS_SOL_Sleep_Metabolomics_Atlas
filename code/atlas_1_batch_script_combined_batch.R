...
rank_normalization_yes<-input_grid[comb_number,"rank_norm"]
rescale_yes<-input_grid[comb_number,"rescale"]
model<-input_grid[comb_number,"model"]
covars<-if (!is.null(input_grid[comb_number,"covariates"])) {unlist(strsplit(input_grid[comb_number,"covariates"],split = " "))} else {na}
stratifier<-input_grid[comb_number,"stratifier"]
...
b1b2_imp_mdl_combine <- b1b2_imp_mdl_append %>%
  dplyr::group_by(metabolite) %>%
  dplyr::summarise(rubins_result = list(rubins_rule(ests = beta, ses = se, round_digit = 2))) %>%
  dplyr::ungroup() %>%
...
b1b2_imp_mdl_total<-rbind(b1b2_imp_mdl_combine[,c("metabolite","beta","se","n","p_val")],b1b2_half_mdl[,c("metabolite","beta","se", "n","p_val")])%>%
  dplyr::mutate(p_val_fdr = p.adjust(p_val, method = "BH"),
                trait=sleep_trait,
                model=model,
                strata="both",
                covar=input_grid[comb_number,"covariates"]) 
# stratified analysis
# non-xenobiotic metabolites
  ...
  # unnest the list column to get the combined estimates in separate columns
  b1b2_imp_strat_female_mdl_combine <- b1b2_imp_strat_mdl_append[which(b1b2_imp_strat_mdl_append$strata=="Female"),] %>%
    dplyr::group_by(metabolite) %>%
    dplyr::summarise(rubins_result = list(rubins_rule(ests = beta, ses = se, round_digit = 2))) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(metabolite = factor(metabolite, levels = unique(metabolite))) %>%
  ...
  b1b2_imp_strat_male_mdl_combine <- b1b2_imp_strat_mdl_append[which(b1b2_imp_strat_mdl_append$strata=="Male"),] %>%
    dplyr::group_by(metabolite) %>%
    dplyr::summarise(rubins_result = list(rubins_rule(ests = beta, ses = se, round_digit = 2))) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(metabolite = factor(metabolite, levels = unique(metabolite))) %>%
  ...
  b1b2_imp_strat_female_mdl_total<-rbind(b1b2_imp_strat_female_mdl_combine[,c("metabolite","beta","se","p_val","strata","n")],b1b2_half_strat_mdl[which(b1b2_half_strat_mdl$strata=="Female"),c("metabolite","beta","se","p_val","strata","n")])%>%
    dplyr::mutate(p_val_fdr = p.adjust(p_val, method = "BH"))
  b1b2_imp_strat_male_mdl_total<-rbind(b1b2_imp_strat_male_mdl_combine[,c("metabolite","beta","se","p_val","strata","n")],b1b2_half_strat_mdl[which(b1b2_half_strat_mdl$strata=="Male"),c("metabolite","beta","se","p_val","strata","n")])%>%
    dplyr::mutate(p_val_fdr = p.adjust(p_val, method = "BH"))
  ...
  write.csv(b1b2_imp_mdl_total,file =paste0("output/main/",comb_number,".csv"),row.names = F)
